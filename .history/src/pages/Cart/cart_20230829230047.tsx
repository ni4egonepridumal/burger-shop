...
export const CartPage = () => {
    ...
    const totalPriceFromState = burgerToCart.reduce((accum, burger) => burger.price * burger.count, 0)
    const totalPriceFromLocalStorage = burgerFromLocalStorage.reduce((accum: number, burger: IBurger) => burger.price + accum, 0)
    const [totalSumm, setTotalSumm] = React.useState<number[]>([])
        ...
        <>
            {burgerFromLocalStorage.length > 0 ?
                <div className={styles.container}>
                    ...
                    <div>Общая стоимость заказа: <span>{totalPriceFromState} руб</span></div>
                </div>
                :
