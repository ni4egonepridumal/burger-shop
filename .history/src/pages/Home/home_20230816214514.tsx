...
export const HomePage = () => {
    ...
    const { data: sort } = useSortBurgerFromNameQuery();
    console.log(data)
    ...
    return (
        <div className={styles.container}>
            ...
            <div className={styles.burger_inner}>
                {isLoading ? <div className={styles.loader}><MoonLoaderfrom size={100} /></div> : burger === "alphabet " ? sort?.map((burger: IBurger) => <BurgerCart key={burger.id} burgers={burger} />) : data?.map((burger: IBurger) => <BurgerCart key={burger.id} burgers={burger} />)}
                {/* {isLoading ? <div className={styles.loader}><MoonLoaderfrom size={100} /></div> :  sort?.map((burger: IBurger) => <BurgerCart key={burger.id} burgers={burger} />)} */}
            </div>
