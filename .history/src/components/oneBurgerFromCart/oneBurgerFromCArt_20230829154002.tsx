...
export const OneBurgerFromCart = ({ burger, totalPrice, setTotalPrice }: IOneBurgerFromCart) => {
    ...
    const [arrL, setArrL] = React.useState<number[]>([]);
    const { burgerToCart } = useAppSelector(state => state)
    console.log("ИТОГ", a)
    console.log("rjkk", arrL)


    // const result = (arr, count) => {
    //     const arr2 = arr.reduce((accum, item) => item.price * count, 0)
    ...
    const handlePlusClick = () => {
        ...
        setA(burgerToCart.reduce((accum, item) => (accum += item.price * burgerItem), 0))
        setArrL([...arrL, a])
    }
