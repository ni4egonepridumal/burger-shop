...
export const ChoiseBurger = ({ popup, setPopup, burgers }: IBurgerPage) => {
    ...
    const funcionfromdataToComment = (data) => {
        fetch('https://64be50915ee688b6250c2e7a.mockapi.io/BroBurger/9', {
            method: 'PUT', // or PATCH
    ...
    let newObj = structuredClone(burgers)

    const addNewComment = (item1, newObj: IBurger) => {
        setResp(newObj.comments.push(item1))
    }
    console.log(resp)
    ...
    const onSubmit: SubmitHandler<InputForm> = (data) => addNewComment(data, newObj)
    return (
