...
export const ChoiseBurger = ({ popup, setPopup, burgers }: IBurgerPage) => {
    ...
    const [resp, setRest] = React.useState({ name: '', comment: '', id: 0 })

    const addNewComment = (item, id) => {
        setRest({ name: item.name, comment: item.comment, id: id })
    }
    console.log(resp)
    ...
    const onSubmit: SubmitHandler<InputForm> = (data) => addNewComment(data, burgers.id)
    return (
        <div className={styles.container}>
            ...
            <button onClick={addComment(resp)}>Передать данные в новый объекет</button>
        </div >
    );
};
