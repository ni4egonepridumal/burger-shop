...
export const BurgerItem: React.FC<IBurgerCartProps> = ({ burgers }) => {
    ...
    console.log('burgetToCart', burgetToCart)
    ...
    const handleClick = () => {
        // if (!choiseBurger) {
        //     addBurgerFromCart()
        // } else { removeBurgerfromCart() }

        !choiseBurger ? removeBurgerfromCart() : addBurgerFromCart()
    }
    ...
    const choiseBurger = burgetToCart.some(burger => burger.id === burgers.id)
    return (
        <div className={styles.burger_cart}>
            ...
            <div className={styles.burger_footer}>
                ...
                {/* <Button viev='primary' size='s' onClick={removeBurgerfromCart}>"Удалить" </Button>
                <Button viev='primary' size='s' onClick={addBurgerFromCart}>"В корзину"</Button> */}
                <Button viev='primary' size='s' onClick={handleClick}>{choiseBurger ? "Удалить" : "В корзину"}</Button>
            </div>
