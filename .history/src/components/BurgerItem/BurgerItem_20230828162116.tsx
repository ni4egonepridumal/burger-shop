...
export const BurgerItem: React.FC<IBurgerCartProps> = ({ burgers }) => {
    ...
    console.log("бургеры в компоненте из Локального стейта, не в СТОРЕ", burgerFromLokalStore)
    const choiseBurger = burgerFromLokalStore.some((burger: IBurger) => burger.id === burgers.id)
    console.log("choiseBurger", choiseBurger)
    return (
