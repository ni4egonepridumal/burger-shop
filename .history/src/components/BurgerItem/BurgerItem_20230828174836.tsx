...
export const BurgerItem: React.FC<IBurgerCartProps> = ({ burgers }) => {
    ...
    let someBurger = [];
    someBurger = JSON.parse(localStorage.getItem("burger"));
    const choiseBurger = someBurger ? someBurger?.some((burger: IBurger) => burger.id === burgers.id) : burgerToCart?.some((burger: IBurger) => burger.id === burgers.id)
    return (
