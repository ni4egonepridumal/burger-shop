...
export const BurgerItem: React.FC<IBurgerCartProps> = ({ burgers }) => {
    ...
    //@ts-ignore
    const burgerFromLocalStorege = JSON.parse(localStorage.getItem("burger"));
    console.log('Бургеры из компонента BurgerItem', burgerFromLocalStorege);

    const choiseBurger = burgerFromLocalStorege?.some((burger: IBurger) => burger.id === burgers.id)
    return (
